...
TEST_CASE("Shortest Travelling Salesman Simple", "[shortestSalesman]") {
  ...
  auto salesman_17_12_15_19 = graph.shortestSalesmanDijkstra({17, 12, 15, 19}).value();
...
TEST_CASE("Large set dijkstra = astar shortest", "[shortestAStar]") {
  ...
  auto shortest_djikstra1 = graph.shortestPathDijkstra(0, 2).value();
  auto shortest_astar1 = graph.shortestPath(0, 2).value();

  REQUIRE(shortest_djikstra1 == shortest_astar1);

  
  auto shortest_djikstra2 = graph.shortestPathDijkstra(0, 10000).value();
  auto shortest_astar2 = graph.shortestPath(0, 10000).value();

  REQUIRE(shortest_djikstra2 == shortest_astar2);

  auto shortest_djikstra3 = graph.shortestPathDijkstra(4000, 15000).value();
  auto shortest_astar3 = graph.shortestPath(4000, 15000).value();
...
TEST_CASE("AStar prefomrance", "[APerf]") {
  ...
  BENCHMARK("Dijkstra small"){
    return graph.shortestPathDijkstra(0, 10);
  };
  ...
  BENCHMARK("Dijkstra big"){
    return graph.shortestPathDijkstra(1000, 50000);
  };
  ...
  BENCHMARK("Dijkstra salesman"){
    return graph.shortestSalesmanDijkstra({0,1,1000,10000, 150, 50000});
  };
...
TEST_CASE("Shortest Path A Star Simple", "[shortestPathAStar]") {
  ...
  auto A_star_path_14_13 = graph.shortestPath(14, 13).value();
  ...
  auto A_star_path_10_18 = graph.shortestPath(10, 18).value();
...
TEST_CASE("Comparing Shortest Path and Shortest Path A star Simple", "[shortestSalesman], [shortestPathAStar]") {
  ...
  auto path_14_13 = graph.shortestPathDijkstra(14, 13).value();
  auto path_10_18 = graph.shortestPathDijkstra(10, 18).value();
  auto path_9_7 = graph.shortestPathDijkstra(9, 7).value();

  auto A_star_path_14_13 = graph.shortestPath(14, 13).value();
  auto A_star_path_10_18 = graph.shortestPath(10, 18).value();
  auto A_star_path_9_7 = graph.shortestPath(9, 7).value();
...
TEST_CASE("Comparing Shortest Path and Shortest Path A star Medium", "[shortestSalesman], [shortestPathAStar]") {
  ...
  auto path_9_11 = graph.shortestPathDijkstra(9, 11).value();
  auto path_18_20 = graph.shortestPathDijkstra(18, 20).value();
  auto path_6_10 = graph.shortestPathDijkstra(6, 10).value();

  auto A_star_path_9_11 = graph.shortestPath(9, 11).value();
  auto A_star_path_18_20 = graph.shortestPathDijkstra(18, 20).value();
  auto A_star_path_6_10 = graph.shortestPathDijkstra(6, 10).value();
...
TEST_CASE("Shortest Travelling Salesman A*", "[shortestSalesman]") {
  rapidcsv::Document nodes("data/NA.cnode.txt",
                           rapidcsv::LabelParams(-1, -1),
                           rapidcsv::SeparatorParams(' '));
  auto x_coords = nodes.GetColumn<double>(1);
  auto y_coords = nodes.GetColumn<double>(2);

  rapidcsv::Document edges("data/NA.cedge.txt",
                           rapidcsv::LabelParams(-1, -1),
                           rapidcsv::SeparatorParams(' '));
  auto from = edges.GetColumn<size_t>(1);
  auto to = edges.GetColumn<size_t>(2);
