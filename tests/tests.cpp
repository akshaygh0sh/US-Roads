...
TEST_CASE("Shortest Travelling Salesman A* Simple", "[shortestSalesman]") {
  ...
  // Path
  REQUIRE(graph.shortestSalesman(path).value().first ==
          graph.shortestSalesmanAStar(path).value().first);
  // Distance of Path
  REQUIRE(graph.shortestSalesman(path).value().second ==
